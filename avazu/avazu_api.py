...
def add_register_user(register_user):
    ...
    try:
        ...
        error_log.write("需要绑定的系统用户(roles_obj): %s\n" % roles_obj)
        need_push_asset = set()
        ...
        rule.role = roles_obj
        rule.save()
        error_log.write(u'%s :规则创建成功\n' % (rule.name))
    except ServerError, e:
        error_log.write(u'用户"%s"添加授权失败\n' % (register_user.name))
        error = u'用户"%s"添加授权失败\n' % (register_user.name)
        raise ServerError
    ...
    register_user.save()
    error_log.write(u'%s :注册用户状态更为为以处理\n' % (register_user.name))
    user_add_mail(user, kwargs=locals())
    error_log.write(u'用户"%s"邮件发送成功\n' % (user.username))
